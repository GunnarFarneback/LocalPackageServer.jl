...
function collect_meta(config::Config)
    # Get a count of how many things we've cached
    cache_dir = config.cache_dir
    num_pkgs = get_num_pkgs_cached(cache_dir)
    num_artifacts = get_num_artifacts_cached(cache_dir)
...
function serve_meta(config, http::HTTP.Stream)
    metadata = collect_meta(config)
